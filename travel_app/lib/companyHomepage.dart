...
class _MyCompanyHomeState extends State<MyCompanyHome> {
  ...
  Widget build(BuildContext context) {
    ...
    final currentInfo =
        selectt(ci, FirebaseAuth.instance.currentUser.uid.toString());
    final reserv = Provider.of<List<Reservation>>(context);
    final room = Provider.of<List<MyRooms>>(context);
    final req = getMyRequests(
        room, reserv, FirebaseAuth.instance.currentUser.uid.toString());
    final res = getMyReservations(
        room, reserv, FirebaseAuth.instance.currentUser.uid.toString());
    return new Scaffold(
        appBar: AppBar(
            toolbarHeight: 60,
                                                              ...
                                                              onPressed: () {
                                                                Navigator.of(
                                                                    ...
                                                                    .push(MaterialPageRoute(
                                                                        builder:
                                                                            (context) =>
                                                                                MyReservationDetails(req[index])));
                                                              }))),
                                                      DataCell(Container(
                                                          child: Column(
                                                              children: [
  ...
  List<Reservation> getMyRequests(
      List<MyRooms> room, List<Reservation> reservations, String u) {
    List<Reservation> l = [];
      ...
      for (i = 0; i < room.length; i++)
        if (room[i].uidHotel == u) {
          for (int j = 0; j < reservations.length; j++)
            if (room[i].uid == reservations[j].tipCamera &&
                reservations[j].status == 'In progress') {
